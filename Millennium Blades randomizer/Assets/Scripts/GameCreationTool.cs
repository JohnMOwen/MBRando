...
public class GameCreationTool : MonoBehaviour
{
    ...
    public static GameCreationTool instance;
    ...
    void Start()
    {
        ...
        List<string> expansionNames = DatabaseHandler.GetFullExpansionList();

        foreach (string expansionName in expansionNames)
        {
            Expansion expansion = new Expansion();
            expansion.ExpansionName = expansionName;
            expansion.mySets = DatabaseHandler.GetCardSetsContainedInExpansion(expansionName).ToArray();
    ...
    public void GenerateRandomSets()
    {
        ...
        List<CardSet> TeamPowers = new List<CardSet>();
        List<CardSet> Bosses = new List<CardSet>();
        List<CardSet> Metagames = new List<CardSet>();
                    ...
                    if (set.IncludeCardSet)
                    {
                        switch (set.SetType)
                        {
                            ...
                            case CardSet.TypeOfSet.Character:
                                Characters.Add(set);
                                break;
                            case CardSet.TypeOfSet.TeamPower:
                                TeamPowers.Add(set);
                                break;
                            case CardSet.TypeOfSet.Boss:
                                Bosses.Add(set);
                                break;
                            case CardSet.TypeOfSet.Metagame:
                                Metagames.Add(set);
                                break;
                        }
                    }
        ...
        List<CardSet> SelectedTeamPowers = new List<CardSet>();
        List<CardSet> SelectedBosses = new List<CardSet>();
        List<CardSet> SelectedMetagames = new List<CardSet>();
        ...
        //TODO: Read how team powers work and implement selection here

        index = Random.Range(0, Bosses.Count);
        SelectedBosses.Add(Bosses[index]);
        Bosses.RemoveAt(index);

        for(int i = 0; i < 2; i++)
        {
            index = Random.Range(0, Metagames.Count);
            SelectedMetagames.Add(Metagames[index]);
            Metagames.RemoveAt(index);
        }
        ...
        header = GameObject.Instantiate(HeaderPrefab, scrollViewContent.transform);
        header.GetComponent<TMP_Text>().text = "Boss Deck";
        GameObject bSet = GameObject.Instantiate(IndividualTextPrefab, scrollViewContent.transform);
        bSet.GetComponent<TMP_Text>().text = SelectedBosses[0].SetName;

        header = GameObject.Instantiate(HeaderPrefab, scrollViewContent.transform);
        header.GetComponent<TMP_Text>().text = "Metagame Decks";
        for (int i = 0; i < SelectedMetagames.Count; i++)
        {
            GameObject set = GameObject.Instantiate(IndividualTextPrefab, scrollViewContent.transform);
            set.GetComponent<TMP_Text>().text = SelectedMetagames[i].SetName;
        }
    }
...
public class ExpansionList
{
    ...
    public static List<Expansion> LoadData()
    {
